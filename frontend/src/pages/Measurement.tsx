...
import { Check, Ruler, ArrowRight, BookmarkPlus, Info } from 'lucide-react';
    ...
    try {
      ...
      if (saveProfile && user) {
        await saveProfileMutation
          .mutateAsync({ name: 'Default Profile', unit, values: numericValues, isDefault: true })
    ...
    <div className="mx-auto max-w-3xl px-3 pt-4 sm:px-5">
      ...
      {user && profiles && profiles.length > 0 ? (
        <section className="mb-4 rounded-xl2 bg-marigold-50 p-3.5">
          <h2 className="mb-2 text-[14px] font-bold text-ink">Aapke saved measurements</h2>
            ...
            <div key={field.key} className="card overflow-hidden">
              <div className="flex gap-3 p-3.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {field.labelHi ? <p className="text-[12.5px] text-maroon-700">{field.labelHi}</p> : null}
                  {field.instruction ? (
                    <p className="mt-1 flex gap-1.5 text-[12.5px] leading-snug text-ink-muted">
                  ...
                  <div className="mt-2.5 flex items-center gap-2">
                    <input
                      ...
                      className={clsx('field w-28 text-center text-lg font-bold', errors[field.key] && 'border-alert')}
      ...
      <div className="mt-4">
        {user ? (
          <label className="flex cursor-pointer items-center gap-3 rounded-xl2 bg-white p-3.5 shadow-card">
            <input
              type="checkbox"
              checked={saveProfile}
            ...
            <span className="flex items-center gap-2 text-[14px] font-semibold text-ink">
              ...
              Yeh measurement save karein (agli baar kaam aayega)
            </span>
          </label>
        ) : (
          <button
            ...
            className="w-full rounded-xl2 bg-white p-3.5 text-left text-[14px] shadow-card"
          >
